...
func _handle_on_play_effects(new_minion: Minion, game_state: GameState, board: Board) -> void:
	...
	if new_minion.has_ability(Abilities.CHALLENGE_ALL) and not game_state.player.board.is_empty():
		...
		for enemy in enemies:
			if new_minion.is_dead() or new_minion not in game_state.opponent.board:
				break
			if enemy not in game_state.player.board:
				continue
	...
	if new_minion.has_ability(Abilities.ON_PLAY_CHALLENGE_WIN_BUFF) and not game_state.player.board.is_empty():
		...
		await board.animate_creature_challenge(new_minion.instance_id, game_state.opponent.player_id, target.instance_id)
		board.refresh()
		if new_minion in game_state.opponent.board and target not in game_state.player.board:
	...
	for ability in new_minion.abilities:
		if Abilities.is_pilot(ability):
			...
			if not mechs.is_empty():
				var mech_target = _pick_best_pilot_target(mechs)
				await get_tree().create_timer(THINK_DELAY).timeout
...
func _attack_with_all(game_state: GameState, board: Board) -> void:
	...
	while made_attack:
		...
		# Filter out Ethereal (combat_immune) targets — attacking them does nothing
		var candidates: Array[Minion] = []
...
func _score_trade(attacker: Minion, target: Minion) -> int:
	...
	if we_kill and we_survive:
		# Best case — remove a threat and keep our minion
		return target.current_attack * 2 + target.current_health + 10
	elif we_kill:
		# Both die — worth it only if target is more valuable than us
		...
		# Chip damage with no removal — almost never worth it
		return -5
	else:
		# Mutually bad — avoid
		return -15
...
# Prefer mechs that gain bonus abilities when piloted
func _pick_best_pilot_target(mechs: Array[Minion]) -> Minion:
	var synergy: Array[Minion] = []
	for m in mechs:
		if m.has_ability(Abilities.ON_PILOTED_GAIN_RUSH) or m.has_ability(Abilities.ON_PILOTED_GAIN_SAFEGUARD):
			synergy.append(m)
	return _pick_strongest(synergy) if not synergy.is_empty() else _pick_strongest(mechs)
